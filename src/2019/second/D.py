...
int main() {
  ...
  for (int qq = 1; qq <= tt; qq++) {
    ...
    int cnt;
    vector<int> scc = find_scc(g, cnt);
    vector<int> vers(cnt, 0);
    vector<vector<int>> all(cnt);
    for (int i = 0; i < n; i++) {
      vers[scc[i]]++;
      all[scc[i]].push_back(i);
    }
    vector<int> inside(cnt, 0);
    for (auto& e : g.edges) {
      if (scc[e.from] == scc[e.to]) {
        inside[scc[e.from]]++;
      }
    }
    vector<Mint> res(cnt, 0);
    vector<int> any(cnt, 0);
    vector<int> inf(cnt, 0);
